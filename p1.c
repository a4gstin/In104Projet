...
// 2. Implement function to add users (with their friendships)
void addUserWithFriendships(const char* name, int friendCount, const char** friendNames, const int* strengths) {
    ...
    for (int i = 0; i < friendCount; i++) {
        ...
        if (friendIndex != -1) {
            ...
            printf("Friend %s not found. Friendship not added.\n", friendNames[i]);
        }
    }
}
...
int main() {
    ...
    // Example: Adding users and friendships
    const char* jeanFriends[] = {"Alice", "David"};
    int jeanStrengths[] = {3, 5};
    addUserWithFriendships("Jean", 2, jeanFriends, jeanStrengths);
    
    const char* aliceFriends[] = {"Jean", "David"};
    int aliceStrengths[] = {3, 2};
    addUserWithFriendships("Alice", 2, aliceFriends, aliceStrengths);
    
    const char* davidFriends[] = {"Jean", "Alice", "Eric"};
    int davidStrengths[] = {5, 2, 1};
    addUserWithFriendships("David", 3, davidFriends, davidStrengths);
    
    const char* ericFriends[] = {"David"};
    int ericStrengths[] = {1};
    addUserWithFriendships("Eric", 1, ericFriends, ericStrengths);
    
    const char* gregoryFriends[] = {"Sandra"};
    int gregoryStrengths[] = {4};
    addUserWithFriendships("Gregory", 1, gregoryFriends, gregoryStrengths);
    
    const char* sandraFriends[] = {"Gregory"};
    int sandraStrengths[] = {4};
    addUserWithFriendships("Sandra", 1, sandraFriends, sandraStrengths);
    ...
    for (int i = 0; i < userCount; i++) {
        ...
        for (int j = 0; j < userCount; j++) {
            if (adjacencyMatrix[i][j] != NO_CONNECTION) {
                printf("%s(%d) ", users[j].name, adjacencyMatrix[i][j]);
            }
        }
        printf("\n");
    }
    ...
    // Find shortest path
    int path[MAX_USERS];
    int start = findUserIndex("Jean");
    int end = findUserIndex("Eric");
    int pathLength = findShortestPath(start, end, path);
    printf("\nShortest path between Jean and Eric: ");
    printPath(path, pathLength);
    
    // Find potential friends at level 2
    int potentialFriends[MAX_USERS];
    int count;
    findPotentialFriends(start, 2, potentialFriends, &count);
    printf("\nPotential friends for Jean at level 2: ");
    for (int i = 0; i < count; i++) {
        printf("%s ", users[potentialFriends[i]].name);
    }
    printf("\n");
    
    // Explore all friendships from Jean
    printf("\nAll friendships reachable from Jean: ");
    int visited[MAX_USERS] = {0};
    exploreAllFriendships(start, visited);
    printf("\n");
    
    // Check if path exists
    bool pathExists = hasPath(start, findUserIndex("Sandra"));
    printf("\nPath exists between Jean and Sandra: %s\n", pathExists ? "Yes" : "No");
    
    // Dijkstra's weakest path
    dijkstraWeakestPath(start, end);
    
    // A* weakest path
    aStarWeakestPath(start, end);
    
    // Connected components
    findConnectedComponents();
    
    // Order users by friend count
    orderUsersByFriendCount();
    
    // Modify friendship
    modifyFriendship("Jean", "Alice", 6);
    printf("\nAfter modifying friendship strength between Jean and Alice to 6:\n");
    for (int i = 0; i < userCount; i++) {
        if (adjacencyMatrix[start][i] != NO_CONNECTION) {
            printf("%s: %d ", users[i].name, adjacencyMatrix[start][i]);
        }
    }
    printf("\n");
